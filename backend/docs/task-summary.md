...
## 2026-07-28 人物移动速度取整修复

- 人物移动速度保持 `80` 像素/秒，不修改网络上报频率、远端玩家插值速度或宠物跟随参数。
- 本地人物每帧整数像素对齐前先保留小数位移余量，下一物理帧继续累加，避免 `80/60` 像素的单帧位移被长期取整为 `1` 像素。
- 服务端权威坐标覆盖本地位置时同步清空小数余量，防止切图或重同步后残留位移。
