...
- **数据库**：生成 `backend/server/migrations/119_world_scene_boundaries.sql`，在 `world_scene_definition` 增加矩形边界与审计字段，初始化 1~26 号场景并添加矩形顺序、坐标范围和原因约束。迁移未执行，需用户按顺序在目标环境手动执行至 `119`。
- **后端实现**：`world.Service` 启动时加载所有启用场景边界；普通移动完成四方向、速度和时间窗校验后，再把高精度位置裁剪到当前场景闭区间。后台更新持久化成功后以复制替换方式原子刷新对应缓存。
- **后台实现**：在现有世界移动配置页增加场景边界表格和固定尺寸编辑弹窗，显示数据库整数与场景格换算值；更新必须填写原因并二次确认，保存后立即展示服务端返回的最终生效值。
- **兼容与范围**：未修改客户端协议和 Godot 场景，未改变传送拓扑；初始矩形已覆盖现有服务端出生/传送坐标。本任务只限制场景外围，不包含墙体、装饰或精细通行区域，后者继续由 P0-06 处理。
- **验证结果**：`GOCACHE=/tmp/pocket-pet-go-build go test ./server/...` 全量后端测试通过；`admin/npm run build` 通过，仅保留 Vite 既有的大包体提示。
- **建议提交信息**：`feat(world): 完成 P0-05 场景权威边界闭环`
