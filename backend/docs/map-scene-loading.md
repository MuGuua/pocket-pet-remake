...
- 玩家可以直接点击标点，或使用屏幕“上一个 / 下一个”按钮、键盘及手柄 `ui_up/ui_down` 循环切换唯一选中态。
...
- 客户端完成地图场景挂载后，先从服务端世界快照的 `nearby_entities` 中筛选 `entity_type=2` 的 NPC，再逐个发送 `NPC_MENU_REQ` 获取玩家当前任务状态对应的动态菜单。
- 当前地图菜单按 `scene_id + entity_id` 缓存在客户端；玩家进入 NPC 碰撞区或从“周围 NPC”列表选择目标时只读取该缓存，不再临时发起菜单请求。
- 预加载期间客户端复用通用 loading 并锁定世界输入。切换地图会清空旧缓存并递增加载代次，旧地图的延迟响应不能写入新地图缓存。
- 单个菜单请求最多等待 5 秒；超时后跳过该 NPC，迟到响应不会弹窗或写入新地图缓存，避免地图输入永久锁定。
