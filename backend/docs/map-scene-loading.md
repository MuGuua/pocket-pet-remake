...
- 当前开放快速传送场景为闪光镇 `scene_id=1..6` 与闪光平原 `scene_id=9..25`。闪光镇“通往闪光平原”以及闪光平原“传送门”“海道”保持可选但不发请求；一次性剧情场景 `scene_id=7` 与中转场景 `scene_id=8` 不写入快速传送配置。
- 迁移 `113_shining_plain_map_teleport_nodes.sql` 为闪光平原 17 张已注册地图写入服务端权威中心出生格；客户端节点只配置 `target_scene_id`，不能提交或覆盖目标坐标。
