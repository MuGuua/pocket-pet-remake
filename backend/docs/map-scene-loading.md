...
- 客户端目标场景脚本先按来源 `portal_id` 选择入口；服务端确认地图拓扑、发布状态和最低进入等级后，才采用该落点并更新玩家场景与位置
...
2. 普通门切图时，实例化目标地图并按来源 `portal_id` 读取入口场景坐标
3. 把目标 `scene_id`、`portal_id` 和可选 `target_pos` 上报服务端
4. 在等待期间锁定移动输入并显示加载态
5. 收到服务端权威结果后装载目标地图场景
6. 根据服务端确认并持久化后的 `self_pos` 放置本地角色
7. 恢复输入并刷新附近实体表现
...
3. 普通门验证通过后采用客户端目标场景脚本提交的入口落点；缺少该字段时使用服务端兼容门点
4. 更新玩家当前 `scene_id` 与位置，并同步目标场景在线索引
5. 返回使用最终落点的 `MOVE_INTENT_RESP`
6. 下发使用同一落点的 `WORLD_RESYNC_PUSH`，并向旁观玩家广播 `ENTITY_ENTER_PUSH`
...
```text
1. 本地角色触发门区或传送点
2. 客户端目标场景脚本按 portal_id 解析入口 target_pos
3. 客户端锁定输入，显示“切换中”
4. 客户端发送 MOVE_INTENT_REQ(scene_id, target_scene_id, portal_id, target_pos)
5. 服务端校验当前场景、门拓扑、目标场景和等级
6. 服务端采用验证通过的普通门 target_pos，持久化并返回 MOVE_INTENT_RESP
7. 服务端下发使用同一坐标的 WORLD_RESYNC_PUSH
8. 客户端收到 WORLD_RESYNC_PUSH 后：
   - 卸载旧地图
   - 挂载新地图
   - 以服务端 self_pos 放置玩家
   - 刷新 AOI/附近实体
...
- 表达“我要通过哪个门切到哪个目标地图”；普通门可同时提交目标场景脚本解析的入口 `target_pos`
...
- 普通门的 `corrected_pos` 使用客户端目标场景脚本选定且经服务端门关系验证后采用的入口坐标；快速传送使用数据库中心点
...
- 服务端只确认本次切图是否允许
- 客户端在请求前按目标场景脚本里的 `portal_id -> 出生场景坐标` 配置选择落点，服务端验证后持久化并以 `self_pos` 回传同一坐标
- 不再假设每张地图只有一个统一出生点
...
- 服务端按 `portal_id` 确认来源场景与目标地图可达，验证通过后采用客户端提交的目标场景入口坐标
- 客户端在目标场景的 `*_level.gd` 中按来源 `portal_id` 读取出生点并作为 `target_pos` 提交；如果未配置或未提交，则服务端使用现有门点作为兼容回退
...
- 客户端不能自行确认切图成功
- 客户端目标场景脚本负责选择普通门入口落点，但不能自行确认该落点生效
...
- 世界根场景常驻
- 地图资源按 `scene_id` 热切换
- 客户端踩中门区后，由目标场景脚本选择入口并随切图意图提交
- 服务端确认门关系和准入后持久化该入口落点
- 客户端收到包含同一落点的权威快照后再真正装载地图
...
- 每次打开地图面板都会读取服务端权威 `GameState.scene_snapshot.scene_id`，通过地区热点已有的 `target_scene_id` 自动进入所属地区；闪光镇 `scene_id=1..6` 与闪光平原 `scene_id=9..26` 会将选择动画、地点名称、焦点和人物当前位置图标同步定位到当前地图热点。
...
- 当前开放快速传送场景为闪光镇 `scene_id=1..6` 与闪光平原 `scene_id=9..26`。闪光镇“通往闪光平原”以及闪光平原“传送门”保持可选但不发请求；海道已映射到 `scene_id=26` 并使用数据库中心格 `(6,7)`，一次性剧情场景 `scene_id=7` 与中转场景 `scene_id=8` 不写入快速传送配置。
- 迁移 `113_shining_plain_map_teleport_nodes.sql` 为闪光平原原有 17 张已注册地图写入服务端权威中心出生格；迁移 `114_shining_plain_seaway.sql` 注册海道并新增 `scene_id=26` 的中心格 `(6,7)`。客户端节点只配置 `target_scene_id`，不能提交或覆盖目标坐标。
...
- 客户端 `WorldSceneRegistry` 将服务端 `scene_id` 映射到对应 `.tscn`；普通门切图时，目标地图脚本还会按来源 `portal_id` 返回入口坐标并随 `target_scene_id + portal_id + target_pos` 提交。
- 闪光平原通用地图在场景根节点的 `portal_spawn_scene_positions` 中维护“来源 `portal_id` -> 当前地图出生场景格”；同一目标地图存在多个入口时必须逐门配置，不能共用默认登录点。
- 门的可达关系、发布状态与最低等级继续由服务端验证；普通门出生格由客户端目标场景脚本维护，服务端验证后将其作为 `corrected_pos` / `self_pos` 持久化和广播。`worldScenes` 门点只作为旧客户端或未配置入口的兼容回退；快速传送中心格仍由数据库权威维护。
