...
- 服务端先按 `portal_id` 确认地图拓扑、发布状态和最低进入等级；客户端收到成功快照并加载目标地图后，再从当前场景脚本读取该来源门的本地入口落点
...
5. 从当前目标地图脚本按来源 `portal_id` 读取导出入口坐标并放置本地角色
6. 恢复输入并刷新附近实体表现
...
3. 忽略跨场景请求中的 `target_pos`，仅保留服务端内部兼容坐标完成持久化和世界快照
...
5. 服务端使用内部兼容坐标完成持久化并返回 MOVE_INTENT_RESP / WORLD_RESYNC_PUSH
6. 客户端收到 WORLD_RESYNC_PUSH 后：
   - 卸载旧地图
   - 挂载新地图
   - 按当前地图脚本的 `portal_id` 导出配置放置本地玩家
   - 将该本地落点初始化为移动同步基线，不把“刚落地”本身上报服务端
   - 刷新 AOI/附近实体
   - 关闭加载态，恢复输入
```
...
- 普通门的 `corrected_pos` 只表示服务端内部兼容位置，客户端本地普通门落点不读取该值；快速传送仍使用数据库中心点
...
- 作为切图后的权威世界快照
- 客户端应以它作为确认目标 `scene_id` 并替换地图资源的依据；普通门本地落点随后从当前场景脚本读取
...
- 服务端只确认本次切图是否允许
- 客户端在服务端确认并加载目标地图后，按当前场景脚本里的 `portal_id -> 出生场景坐标` 导出配置放置本地人物；该坐标不进入协议
- 不再假设每张地图只有一个统一出生点
...
- 服务端按 `portal_id` 确认来源场景与目标地图可达，不接收也不采用客户端地图入口坐标
- 客户端加载目标地图后，在当前 `*_level.gd` 实例中按来源 `portal_id` 读取导出出生点；未配置专属入口时回退同一场景脚本的统一出生点
...
- 客户端不能自行确认切图成功
- 服务端负责确认普通门切图是否成功；客户端只在成功加载目标地图后读取场景导出入口落点
- 服务端必须是最终 `scene_id`、准入结果与内部世界快照的权威来源；普通门本地显示落点由场景导出变量控制
...
- `world_map_teleport_node` 继续保存开放状态和服务端快速传送中心格；客户端只提交 `target_scene_id`，服务端校验并持久化权威场景及坐标后，通过 `MOVE_INTENT_RESP -> WORLD_RESYNC_PUSH` 驱动黑屏转场。目标地图实际加载完成后，客户端人物显示落点改为直接读取该地图根脚本导出的 `login_and_map_teleport_spawn_position`。
- 当前开放快速传送场景为闪光镇 `scene_id=1..6`、闪光镇传送区 `scene_id=8` 与闪光平原 `scene_id=9..26`。闪光镇“通往闪光平原”标点指向 `scene_id=8`；服务端使用数据库中心格 `(5,10)` 完成准入、场景切换和位置持久化，客户端加载后使用闪光镇传送区场景脚本自己的 `login_and_map_teleport_spawn_position` 作为实际显示落点。闪光平原“传送门”仍保持可选但不发请求；一次性剧情场景 `scene_id=7` 不写入快速传送配置。
- 迁移 `113_shining_plain_map_teleport_nodes.sql` 为闪光平原原有 17 张已注册地图写入服务端快速传送中心格；迁移 `114_shining_plain_seaway.sql` 注册海道并新增 `scene_id=26` 的中心格 `(6,7)`；迁移 `116_enable_shanguang_transfer_area_map_teleport.sql` 开放 `scene_id=8` 并配置服务端中心格 `(5,10)`。客户端请求仍只配置 `target_scene_id`，统一出生点不加入请求载荷，而是在场景加载后从本地地图脚本读取。
- 传送到当前地图时，服务端流程仍按数据库中心格完成确认，但客户端人物会在快照应用阶段直接显示到当前地图脚本的统一出生点；不会重复触发离开/进入 AOI 或 `ENTER_SCENE` 任务事件。
...
- 客户端 `WorldSceneRegistry` 将服务端 `scene_id` 映射到对应 `.tscn`；普通门请求只提交 `target_scene_id + portal_id`，目标地图加载后才从当前场景脚本读取入口坐标。
- 闪光平原通用地图在场景根节点的 `portal_spawn_scene_positions` 中维护“来源 `portal_id` -> 当前地图出生场景格”；同一目标地图存在多个入口时必须逐门配置，不能共用默认登录点。
- 门的可达关系、发布状态与最低等级继续由服务端验证；普通门出生格完全由客户端目标场景脚本维护，不提交服务端，也不读取服务端 `corrected_pos` / `self_pos` 作为本地门落点。`worldScenes` 门点仅服务于服务端内部持久化与多人快照兼容；快速传送中心格仍由数据库权威维护。
- 已对全部启用传送门的目标出生格执行 TileMap 物理检查；报名区进入准备区使用 `(2,7)`，闪光南路进入家族会馆使用 `(12,10)`；海道链路使用 `23 -> 26 (6,2)`、`26 -> 23 (8,11)`、`26 -> 25 (2,8)`、`25 -> 26 (10,8)`，避免角色落在地图外或阻挡格。
