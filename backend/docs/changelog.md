...
## 2026-07-27
- 客户端进入地图时会按服务端世界快照提前加载当前场景全部 NPC 动态菜单；玩家进入 NPC 碰撞区后直接读取该地图缓存，不再临时发送菜单请求。
- NPC 菜单缓存严格绑定场景 ID，切图时清空并重建；预加载期间锁定世界输入并复用通用 loading，避免玩家在菜单尚未准备完成时触发交互。
- 剧情确认后在同一地图新解锁的 NPC 会根据最新世界快照自动补载菜单；单请求 5 秒超时会停止等待，迟到响应不会污染新地图缓存。
