...
## 2026-08-22 普通移动改为客户端自主上报并恢复本地战斗坐标

- 普通同场景移动改为客户端自主完成；客户端单向上报整数格、高精度坐标、朝向和起停状态，发送成功后立即更新本地上报基线，不再等待普通移动回包或执行服务端纠偏。
- 服务端普通移动不再调用速度、边界和导航计算，只校验玩家、会话、场景与严格递增的 `move_seq`，随后通过 Redis CAS 记录客户端最近上报状态并向同场景其他玩家发送 `ENTITY_MOVE_PUSH`。旧会话、错误场景、重复/倒退序号和仓储异常均静默丢弃，不向上报者发送 `MOVE_INTENT_RESP` 或 `WORLD_RESYNC_PUSH`。
- 客户端上报的 `precise_pos` 不再限制在整数格半格范围内，服务端原样保存和广播；Redis dirty、停止事件最终写回、周期批量持久化和位置版本保护继续保留。
- 普通传送门、地图快速传送、跨场景切换、登录和重连仍使用服务端权威落点及既有响应/重同步链路，不受普通移动调整影响。
- 客户端在收到战斗开始事件时记录当前本地场景坐标，卸载战斗界面后优先恢复该坐标；记录缺失或场景不匹配时使用当前地图 `get_login_and_map_teleport_spawn_position()` 的传送出生坐标。新客户端不再消费战斗结果中的服务端 `return_pos`。
- 本次无数据库结构、迁移、后台页面或依赖变更；验证结果见同日任务总结。

## 2026-08-18 任务系统领取、推进、交付与任务页链路修复

- 完整核对客户端任务页、任务控制器、协议路由、服务端任务领域服务、NPC 结构化对话副作用和统一奖励服务；任务列表 `6001/6002`、增量推送 `6011/6012`、领取 `6021/6022`、交付 `6031/6032`、追踪 `6041/6042` 的两端契约保持一致。
- 正式 PostgreSQL 只读检查确认主线任务 `1101`～`1103` 的 `objectives_json` 均被覆盖为空数组，`1102`、`1103` 的 `rewards_json` 也为空；玩家 `10001` 已领取 `1101`，但没有 `player_quest_objective`，因此 `TALK_TO_NPC` 事件无法把任务推进到 `READY_TO_SUBMIT`。
- 新增并执行迁移 `backend/server/migrations/124_repair_main_journey_quest_flow.sql`：仅在配置为空时恢复三条主线的对话目标和金币奖励，并为已经处于 `ACCEPTED/READY_TO_SUBMIT` 的玩家幂等补齐缺失目标，不覆盖现有完成提示或非空运营配置。执行结果为更新 3 条模板、补齐 1 条玩家目标。
- `quest.Service.Accept` 对已经领取、待提交或已完成的任务改为幂等返回当前权威快照，重复点击、网络重试或剧情节点重放不再把目标进度重置为 0；新增测试同时覆盖 `ACCEPTED` 与 `READY_TO_SUBMIT`。
- NPC 结构化对话与旧 `quest_submit` 菜单入口在奖励发放失败时都会调用 `RevertCompletedQuestToReady`，避免任务停在 `COMPLETED` 却没有获得奖励，玩家可在背包空间释放或临时故障恢复后重新交付。
- NPC 动态任务菜单现在等待领取/交付回包并保持通用 loading，只有服务端持久化成功后才重新拉取菜单；任务领取、交付、追踪以及 NPC 菜单、动作、剧情的业务拒绝统一通过现有 `ConfirmPromptPopup` 提示框展示。
- 验证通过：任务/WS 专项测试、`GOCACHE=/tmp/pocket-pet-quest-go-cache go test ./server/... -count=1`、`go vet ./server/...`、Godot 4.7 Headless 全项目解析与 `git diff --check`。执行迁移前已将三张受影响表的目标记录备份到 `/tmp/pocket-pet-quest-migration-124-backup-20260818-170857`；迁移后确认 `1101`～`1103` 均有 1 个目标和 1 条奖励，玩家 `10001` 的任务 `1101` 已补齐 `0/1` 对话目标。

## 2026-08-18 战斗宠物成长 SQL 参数类型修复

- 修复 PVE 战斗奖励已经提交后，宠物经验与战斗属性写回在 PostgreSQL 解析 `LEAST` 表达式时触发 `SQLSTATE 42P08` 的问题；根因是同一占位参数同时被推导为 `TEXT` 与 `INTEGER`。
- `savePetExpProgressionQuery` 对当前 HP 与新 HP 上限参数显式使用 `INTEGER` 转换，战斗结算可继续完成宠物经验、等级、属性和当前血量持久化，不再进入“奖励已发放但同步异常”的保护分支。
- 同步修复手动属性加点和批量战斗属性重算中的相同 `LEAST` 参数推导隐患，保持 `player_pet.hp`、`player_pet.hp_max` 的数据库整数契约一致。
- 新增仓储专项测试锁定三条 SQL 的显式类型约束；已连接当前 PostgreSQL，在只读结构检查及回滚事务内验证三条语句均可成功 `PREPARE`，未修改任何数据库数据或结构。
- 验证通过：`go test ./server/internal/data/postgres ./server/internal/transport/ws -count=1`、`go test ./server/... -count=1`、`go vet ./server/...` 与 `git diff --check`。本次无数据库迁移、协议、客户端或依赖变更，原有战斗奖励幂等保护保持不变。
...
## 2026-08-22 场景切换黑屏关键路径优化

- 服务端切图改用 `WorldTransferProfile` 轻量读取玩家等级、场景、坐标和位置版本，不再为传送请求刷新或读取完整战斗快照。
- Redis 切图移动序号推进复用 handler 已加载状态，由重复 `GET → GET → CAS → GET` 收敛为 `GET → CAS`；PostgreSQL 位置仍按版本同步落库，未改变切图成功的一致性语义。
- 客户端在发出传送请求时线程预加载目标地图，黑屏中点优先消费预加载资源，加载失败或资源不匹配时沿用同步加载兜底。
- `scene_loaded` 现在只等待地图、人物、服务端权威落点和相机等视觉关键步骤；导航网格在场景可见后按每帧最多 64 格分帧构建，并通过 `scene_runtime_ready` 标记完整运行态就绪。
- 导航构建期间仅暂时禁用点击寻路和暗雷步数判断，手动移动继续使用本地物理碰撞；远端人物与跟随宠物延后到 `scene_loaded` 后补齐，不再延长黑屏。
- 服务端新增 `[SceneTransition][Server][Timing]` 分段日志，客户端沿用 `[SceneTransition][Client][World]` 记录预加载、地图挂载和导航构建耗时，便于继续定位数据库、Redis、快照与资源加载瓶颈。
- 普通同场景移动仍保持客户端自主、仅向服务端上报并广播给其他玩家，不向本机回包或纠错；战斗前本地坐标记录与战斗后本地恢复逻辑保持不变。
- 本次没有新增协议字段、数据库迁移或第三方依赖。
