...
- 正式登录与世界地图快速传送共用的 `login_and_map_teleport_spawn_position`，以及普通门按来源 `portal_id` 配置的入口坐标，都是客户端地图场景导出配置，不属于协议字段。客户端只在服务端确认目标场景并加载地图后读取并应用本地人物落点
...
- `speed`：服务端确认的权威移动速度；协议字段已预留，权威速度接入前零值表示尚未提供
