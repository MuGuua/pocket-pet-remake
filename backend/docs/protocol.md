...
- 普通同场景移动由客户端自主完成并单向上报当前位置；传送门、地图快速传送和跨场景切换仍只提交意图，由服务端决定最终场景与落点
...
- `self_precise_pos` 仅在断线重连存在 Redis 在线移动状态时返回，使用千分之一场景格定点整数；该值是客户端最近一次成功上报的位置，客户端应优先于 `self_pos` 恢复人物位置。首次进入世界或 Redis状态缺失时省略，并使用 PostgreSQL来源的 `self_pos`。
...
`MOVE_INTENT_REQ` 同时承载门区切图、世界地图快速传送和同场景位置上报。普通门切图只使用 `target_scene_id`、`portal_id`，不携带地图出生落点；地图快速传送额外传 `map_teleport=true`，同样不携带本地出生点；同场景移动由客户端自主完成，`target_pos` 与 `precise_pos` 是客户端当前实际坐标。跨场景请求一律忽略客户端坐标。
...
- `target_pos`：客户端当前同场景整数坐标；服务端通过会话、场景和 `move_seq` 校验后记录该值，用于远端玩家同步和异步持久化，不再计算或纠正本机普通移动。普通门和世界地图快速传送都会忽略该值
- 正式登录、普通门和世界地图快速传送的唯一人物落点均来自服务端权威快照 `self_pos`；客户端地图加载完成后只做场景坐标到 Godot 像素坐标的转换。历史场景导出出生配置不属于协议字段，也不再覆盖服务端快照
- `precise_pos`：千分之一场景格的当前表现坐标；服务端原样保存和广播，缺失时才使用 `target_pos * 1000` 兼容补齐
- `facing`：四方向单位向量，用于其他玩家视角中的人物和宠物表现；缺失或非法时服务端按整数格位移方向兼容推导，但不会因此拒绝或纠正本机移动
- `moving`：明确人物正在行走或已经停止；旧客户端缺少该字段时由服务端按整数格变化兼容推导
- `input`：可选的四方向移动输入表现字段；停止时为零向量。当前仅保留协议兼容和诊断用途，不参与服务端普通移动计算
- `client_tick`：可选的客户端单调时钟毫秒数，只用于延迟诊断，不参与服务端权威计时或速度计算
- 普通同场景上报是 fire-and-forget：服务端不会发送 `MOVE_INTENT_RESP`，客户端也不维护等待回包状态
- 旧会话、错误场景、重复或倒退 `move_seq`、Redis 短时异常等问题只会让服务端静默丢弃本次普通移动上报，不会向本机发送拒绝、重同步或坐标纠正

### 2013 ENTITY_MOVE_PUSH

服务端成功记录普通移动上报后，仅向同场景其他玩家推送。`to_pos` 是移动者上报的整数坐标，`precise_pos`、`facing`、`moving` 用于远端人物与宠物的实时表现；该推送不是发给移动者本机的权威纠偏。
...
- `scene_version`：来自移动者当前 Redis 在线移动状态；同地图快速传送使用服务端传送决策中的目标场景代次，不再写死固定值。未装配移动状态仓储的旧服务兼容链路可返回 `0`，客户端会按当前全量世界快照的场景代次解释。
...
`MOVE_INTENT_RESP` 仅用于普通传送门、地图快速传送和跨场景切换。普通同场景位置上报不返回该消息，新客户端也不会用该消息纠正本机普通移动。
...
```json
{
  ...
  "server_tick": 1786766400123,
  "speed": 3750,
  "correction_ignore_distance": 125,
  "correction_snap_distance": 1125,
  "reason": ""
}
```
...
- `corrected_pos`：服务端最终采用并持久化的传送落点；普通门使用服务端拓扑目标点，快速传送使用数据库中心点。切图后的 `WORLD_RESYNC_PUSH.self_pos` 与该位置保持一致，客户端不得覆盖
- `corrected_precise_pos`：传送完成后的千分之一场景格落点；兼容旧客户端保留
- `server_tick`、`speed`、`correction_ignore_distance`、`correction_snap_distance`：旧版普通移动纠偏兼容字段；新客户端不使用这些字段纠正同场景移动
- 如果 `target_scene_id` 为空、为 `0`、或等于当前 `scene_id` 且不是快速传送，服务端按普通同场景位置上报处理并且不发送响应
...
`self_pos` 为客户端当前场景坐标（与 `ENTER_WORLD_RESP.self_pos` 同坐标系），保留用于遭遇请求兼容。服务端仍可在战斗结果中提供 `return_pos` 兼容旧客户端，但新客户端在进入战斗前记录本地场景坐标，离开战斗后直接恢复该本地坐标；记录缺失或场景不匹配时，使用当前地图 `get_login_and_map_teleport_spawn_position()` 返回的传送出生坐标，不消费服务端 `return_pos`。
