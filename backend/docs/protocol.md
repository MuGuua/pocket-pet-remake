...
`MOVE_INTENT_REQ` 同时承载门区切图、世界地图快速传送和同场景移动意图。普通门切图只使用 `target_scene_id`、`portal_id`，不携带地图出生落点；地图快速传送额外传 `map_teleport=true`，同样不携带本地出生点；同场景移动以 `input` 作为服务端权威计算输入，`target_pos` 与 `precise_pos` 仅作为候选位置和旧客户端兼容字段。跨场景请求一律忽略客户端坐标。
...
- `target_pos`：同场景移动候选整数格；服务端不会直接信任该值，而是从 Redis 最新权威位置按输入、数据库速度、时间窗、场景边界和发布导航计算最终位置。普通门和世界地图快速传送都会忽略该值
- 正式登录、普通门和世界地图快速传送的唯一人物落点均来自服务端权威快照 `self_pos`；客户端地图加载完成后只做场景坐标到 Godot 像素坐标的转换。历史场景导出出生配置不属于协议字段，也不再覆盖服务端快照
- `precise_pos`：千分之一场景格的候选表现坐标；服务端只用于误差校验并按权威规则裁剪，不直接作为最终位置或直接写入数据库
- `facing`：四方向单位向量，只接受上下左右，服务端拒绝斜向表现数据并回退为位移方向
- `moving`：明确人物正在行走或已经停止；旧客户端缺少该字段时由服务端按整数格变化兼容推导
- `input`：可选的四方向移动输入意图；停止时为零向量。该字段是同场景权威位移计算的主要输入；旧客户端缺失时服务端只做兼容归一化，不把客户端候选坐标升级为权威规则
...
- `corrected_precise_pos`：服务端确认的千分之一场景格权威表现坐标；本机只在响应 `move_seq` 匹配当前普通移动请求时消费。兼容旧响应时，缺少有效精确坐标会回退到 `corrected_pos * 1000`
- `server_tick`：服务端生成本次权威移动结果的时间基线；客户端记录最近有效值，当前不使用客户端时钟反推权威位移
- `speed`：服务端确认的权威移动速度，单位为“千分之一场景格/秒”；例如 `3750` 表示每秒 `3.75` 个场景格，中误差平滑纠偏只使用该值换算表现速度
- `correction_ignore_distance`：本机预测误差可直接忽略的最大距离，单位为千分之一场景格；当前复用数据库 `world_movement_config.axis_tolerance_milli`
- `correction_snap_distance`：本机预测必须立即吸附的最小距离，单位为千分之一场景格；当前由数据库 `speed_milli_cells_per_second * max_elapsed_ms / 1000` 派生，并保证严格大于忽略阈值
- 本机纠偏分级：误差不超过忽略阈值时保留预测；位于两阈值之间时保留实时输入并逐帧消化固定偏移；达到吸附阈值或服务端拒绝时立即应用权威位置。场景不一致时不换算另一张地图坐标，继续由 `WORLD_RESYNC_PUSH` 重建
- 旧服务未提供有效纠偏阈值时，客户端保留本地预测，不使用硬编码距离兜底；中误差缺少有效权威速度时保守立即应用权威位置
