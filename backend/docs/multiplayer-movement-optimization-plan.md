...
持久化必须携带单调递增的 `position_version`，数据库更新只能由更高版本覆盖较低版本，避免旧批次回写覆盖新位置。
...
- [~] P0-03 实现服务端 `move_seq`、会话代次和场景代次校验。进入世界初始化、切图重置和普通移动入口已接入领域预检与 Redis CAS；跨场景请求的重复序号拒绝仍待补齐。
- [ ] P0-04 实现服务端四方向输入归一化、速度和最大时间跨度校验。
- [ ] P0-05 增加场景边界数据迁移、仓储、后台维护和运行时缓存。
- [ ] P0-06 增加场景静态通行数据迁移、Godot 导出工具、后台发布与回滚。
- [ ] P0-07 将移动判定从 WebSocket handler 下沉到 `world` 领域服务。
...
- [~] P1-03 进入世界时实现 Redis优先、PostgreSQL回退的恢复流程。首次进入以 PostgreSQL快照初始化，同会话重连会保留 Redis状态，缓存缺失时回退 PostgreSQL；重连响应直接采用 Redis最新场景和坐标尚未完成。
...
- [ ] DOC-02 更新架构文档中的 Redis实时位置和 PostgreSQL写回职责。
