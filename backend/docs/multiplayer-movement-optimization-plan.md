...
- [x] P0-10 补充正常、瞬移、越界、穿墙、重复、倒退和切图竞态测试。领域层既有用例覆盖正常权威推进、服务端时间窗瞬移裁剪、场景边界裁剪、静态阻挡路径裁剪、重复/倒退序号与权威会话/场景不匹配；WebSocket 层新增成功移动后倒退序号无副作用测试，以及切图完成后旧场景延迟移动包竞态测试，确认拒绝包不会再次推进 Redis、回滚 PostgreSQL、广播旧坐标，并会向移动者返回新场景权威位置与 `WORLD_RESYNC_PUSH`。本批只补测试和文档，不修改协议、运行逻辑或持久化结构。
