...
func applySkillPassives(actor *actorRuntime) bool {
	return applySkillPassivesWithMode(actor, true)
}

// applySkillPassivesWithoutPersistentStats 只叠加战斗期效果，不重复叠加已经固化进属性面板的永久属性。
func applySkillPassivesWithoutPersistentStats(actor *actorRuntime) bool {
	return applySkillPassivesWithMode(actor, false)
}

func applySkillPassivesWithMode(actor *actorRuntime, includePersistentStats bool) bool {
	...
	for _, skillID := range actor.skillIDs {
		...
		applyPassiveSkillDefWithMode(actor, def, includePersistentStats)
		applied = true
	}
	return applied
}
...
func applyPassiveSkillDef(actor *actorRuntime, def skillDef) {
	applyPassiveSkillDefWithMode(actor, def, true)
}

// applyPassiveSkillDefWithMode 允许战斗入口按需跳过“已提前参与属性计算”的永久面板加成。
func applyPassiveSkillDefWithMode(actor *actorRuntime, def skillDef, includePersistentStats bool) {
	name := def.Name
	switch {
	case strings.HasPrefix(name, "强壮"):
		if includePersistentStats {
