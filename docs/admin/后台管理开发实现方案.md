...
- 筛选：玩家 ID、昵称、状态
- 详情：基础属性、自由属性点、四维已分配点数、背包摘要、宠物摘要、任务摘要
- 操作：封禁、解封、资源修正

### 6.4 玩家成长配置

- 等级经验表：1~100 级每级 `exp_required`、升级奖励属性点（默认每级 1 点）、启用状态
- 属性转化率表：四维源属性到战斗属性的 `convert_rate` 与启用状态
- 操作：单条编辑；保存后服务端立即刷新运行时缓存
- 权限：`player_progression:view`、`player_progression:edit`
- 详细设计见 `backend/docs/player-progression.md`
...
- `GET /api/admin/player-progression/level-configs`：玩家等级经验配置列表
- `PUT /api/admin/player-progression/level-configs/{level}`：更新指定等级经验与升级属性点
- `GET /api/admin/player-progression/attr-convert-configs`：四维属性转化率配置列表
- `PUT /api/admin/player-progression/attr-convert-configs/{id}`：更新单条转化率配置
