...
`quest_template.accept_animation_key` 和 `quest_template.submit_animation_key` 保存客户端剧情注册键，不保存 Godot 资源路径。任务领取、交付与奖励仍由服务端先完成校验和持久化，动画只是成功响应后的客户端表现；断线、资源缺失或动画中断均不回滚任务状态和奖励。

## 《主线·旅行的起点》第一阶段

- `1101`：桃子提供接受/取消选择；接受并完成对话后前往市场，与生产导师·璃梦对话，自动交付并发放 `100` 金币。
- `1102`：前置任务为 `1101`；生产导师·璃梦提供接受/取消选择，对话结束后自动交付并发放 `150` 金币。
- `1103`：前置任务为 `1102`；杂货商人·罗格提供接受/取消选择，对话结束后自动交付并发放 `200` 金币。
- 三个任务菜单都使用结构化 NPC 对话入口，客户端不能绕过“接受”选项直接改变任务状态。
