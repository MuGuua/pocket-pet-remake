...
- 新增或调整普通传送门落点时，在目标地图脚本中维护来源 `portal_id -> 场景坐标`，并确认来源门提交的 `target_scene_id` 与服务端拓扑一致。场景坐标使用有符号整数，允许配置负数入口格；服务端只验证门拓扑和准入条件，不得把坐标正负号当作有效性判断。服务端 `worldScenes` portals/entries 继续保留可达关系与旧客户端兼容落点，但不再是新客户端普通门出生格的唯一配置来源。
