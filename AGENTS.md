...
- 所有ui都必须使用场景和节点数，禁止使用脚本生成ui,避免在脚本中重新设置ui节点的尺寸，最多设置出现位置，方便开发者调整
- 尽量避免使用 scale 的方式放大，先检查能否改变ui的各节点尺寸
- 操作godot节点时优先使用godot ai mcp，如果没连接请检查连接，如果连接失败，请提示用户并使用godot-dev-guide技能。
...
- **GDScript 缩进必须统一使用空格，禁止 Tab 与空格混用。** Godot 会报错：`Used tab character for indentation instead of space as used before in the file.` 该问题在 AI 批量编辑、复制粘贴或合并代码时反复出现，提交前必须自检。
  ...
  - 任何详情展示页，包括但不限于技能介绍，宠物介绍，武器介绍，提示内容，都必须支持富文本格式，统一由后端提供数据
...
- 任何文本编辑器，包括但不限于技能介绍，宠物介绍，武器介绍，提示内容，都必须支持富文本格式，并且需要编写友好。
格式示例：
一瓶非常普通的药水，但却旅行冒险必需品
使用后效果 :
...
- 所有仅用于**运行时弹出/打开**的 Godot UI 场景（例如 `CanvasLayer` 弹窗、背包面板、人物信息面板、奖励弹窗等），**场景资源默认必须是隐藏状态**，避免在编辑器中打开任意场景时出现“面板常驻显示”的误判。
- 这类场景的根节点若是 `CanvasLayer`、`Control` 或其他 UI 根节点，默认应显式写上 `visible = false`；运行时再通过脚本中的 `open_menu()`、`show()` 等正式入口打开。
- 如果某个 UI 需要在运行时首次 `_ready()` 时先执行 `hide()`，仍然**不能**省略场景资源层的默认隐藏；否则编辑器预览、子场景引用预览、多人协作合并后都可能再次出现“任何场景都能看到该面板”的问题。
- 对于 `null_equipment_slot.tscn`、普通按钮、卡槽、图标格子这类**组件型子场景**，默认可见通常是合理的；需要重点检查的是“整块面板 / 整层 HUD / 弹窗”这类运行时容器。
