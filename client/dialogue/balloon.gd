...
const NAME_PANEL_LEFT_MARGIN := 0.0
const NAME_PANEL_RIGHT_MARGIN := 20.0
...
## The panel that frames the speaking character's name
...
func apply_dialogue_line() -> void:
	...
	var resolved_player_portrait: Texture2D = _get_player_portrait()
	var speaker_portrait: Texture2D = resolved_player_portrait if is_player_speaking else npc_portrait

	character_label.visible = not dialogue_line.character.is_empty()
	character_label.clear()
	character_label.fit_content = true
	character_label.autowrap_mode = TextServer.AUTOWRAP_OFF
	portrait_rect.texture = speaker_portrait
	portrait_rect.hide()
	if is_player_speaking:
		character_label.append_text(speaker_name)
		if speaker_portrait != null:
			character_label.append_text(" ")
			character_label.add_image(speaker_portrait, NAME_ICON_SIZE, NAME_ICON_SIZE)
	else:
		if speaker_portrait != null:
			character_label.add_image(speaker_portrait, NAME_ICON_SIZE, NAME_ICON_SIZE)
			character_label.append_text(" ")
	...
	# Show our balloon before measuring so right-aligned name boxes can use the final width.
	balloon.show()
	will_hide_balloon = false

	await get_tree().process_frame
	if character_label.visible:
		var panel_width: float = character_label.get_content_width() + NAME_PANEL_PADDING
		npc_name_panel.custom_minimum_size.x = panel_width
		npc_name_panel.size.x = panel_width
		if is_player_speaking:
			npc_name_panel.position.x = balloon.size.x - panel_width - NAME_PANEL_RIGHT_MARGIN
		else:
			npc_name_panel.position.x = NAME_PANEL_LEFT_MARGIN
		npc_name_panel.show()
	else:
		npc_name_panel.custom_minimum_size.x = 0.0
		npc_name_panel.hide()
...
func _is_player_speaker(speaker_name: String) -> bool:
	var stripped_name := speaker_name.strip_edges()
	var configured_player_name: String = _get_player_name().strip_edges()
	if configured_player_name.is_empty():
		return false
	return stripped_name == configured_player_name
...
func _get_player_portrait() -> Texture2D:
	if _has_some_global():
		var some_global: Node = SomeGlobal
		var configured_portrait: Variant = some_global.get("player_portrait")
		if configured_portrait is Texture2D:
			return configured_portrait
