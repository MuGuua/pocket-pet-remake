...
## 右下角底部菜单抽屉控制器，统一管理业务按钮的收起与展开表现。
@onready var bottom_menu_drawer: BottomMenuDrawer = %BottomMenuDrawer
## 底部单条业务提示正文。
...
func set_auto_encounter_button_state(available: bool, active: bool) -> void:
    ...
    if bottom_menu_drawer != null:
        bottom_menu_drawer.set_auto_encounter_available(available)
    else:
