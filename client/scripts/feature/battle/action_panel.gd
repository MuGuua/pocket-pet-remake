...
## 战斗技能表现资源注册表；选择技能时按服务端 skill_visual_id 读取本地图标。
...
func _refresh_choice_page() -> void:
    ...
    for index: int in range(start_index, end_index):
        ...
        button.icon = _resolve_choice_icon(entry)
        button.visible = true
        button.disabled = false
    _update_choice_page_controls(page_count)

## 技能选择项按表现资源 ID 获取图标；其他选择列表保持纯文字展示。
func _resolve_choice_icon(entry: Dictionary) -> Texture2D:
    if _active_list_type != "skill" or _skill_visual_registry == null:
        return null
    var skill_visual_id: String = str(entry.get("skill_visual_id", "")).strip_edges()
    if skill_visual_id.is_empty():
        return null
    return _skill_visual_registry.get_skill_icon(skill_visual_id)
