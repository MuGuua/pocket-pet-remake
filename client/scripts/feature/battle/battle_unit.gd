...
func set_target_arrow_visible(should_show: bool, arrow_type: String = SELECTION_ARROW_ENEMY) -> void:
	if should_show:
...
func _build_target_arrow_texture() -> ImageTexture:
	...
	var top_y: int = 2
	var center_x: int = int(float(width) / 2.0)
...
func _set_hp_bar_visible(should_show: bool) -> void:
	_ensure_scene_nodes()
	if _status_root != null:
		_status_root.visible = should_show
	if _hp_bar != null:
...
func _play_chj_skill_animation() -> bool:
	...
	var completion_state: Dictionary = {"completed": false}
	...
	while not bool(completion_state.get("completed", false)) and elapsed < wait_sec:
		await get_tree().process_frame
