...
const DEFAULT_EFFECT_BASE_SCALE: Vector2 = Vector2(0.7, 0.7)

## 战斗场景导出的特效整体倍率；只改变客户端演出大小，不改变服务端伤害结算。
static var effect_scale_multiplier: float = 1.0

## 接收 battle_scene.tscn 的导出参数，运行时生成的新特效会使用最新倍率。
static func configure_effect_scale_multiplier(multiplier: float) -> void:
    effect_scale_multiplier = clampf(multiplier, 0.1, 4.0)

## 返回当前命中特效整体根节点缩放，集中处理倍率便于新开发者排查特效大小来源。
static func get_effect_base_scale() -> Vector2:
    return DEFAULT_EFFECT_BASE_SCALE * effect_scale_multiplier

## 返回特效资源自身缩放，避免整体倍率在根节点和精灵节点上被重复放大。
static func get_effect_frame_scale() -> Vector2:
    return DEFAULT_EFFECT_BASE_SCALE
...
func _apply_visual(skill_visual: SkillVisualConfig) -> void:
    ...
    _pivot.scale = get_effect_base_scale()
    ...
    if has_sprite_frames:
        _effect_sprite.scale = Vector2(
            skill_visual.effect_scale.x * get_effect_frame_scale().x,
            skill_visual.effect_scale.y * get_effect_frame_scale().y
        )
