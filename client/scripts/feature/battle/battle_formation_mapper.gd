...
## 战斗站位布局：按单方单位数量在「单列上下」与「组队前后」两种模式间切换。
...
static func build_slot_positions() -> Dictionary:
	...
	var battle_area: Dictionary = {
		...
		"ally_back_x": ALLY_BACK_X,
		"ally_front_x": ALLY_FRONT_X,
		"enemy_front_x": ENEMY_FRONT_X,
		"enemy_back_x": ENEMY_BACK_X,
		"ally_rows": 4,
		"enemy_rows": 6,
	...
	result["left_front"] = result.get("left_front_1", Vector2(ALLY_FRONT_X, MAGIC_CIRCLE_CENTER_Y))
	result["right_front"] = result.get("right_front_1", Vector2(ENEMY_FRONT_X, MAGIC_CIRCLE_CENTER_Y))
	return result
...
static func _assign_side_positions(side_units: Array[Dictionary], is_ally: bool) -> void:
	if side_units.is_empty():
		return
	var front_x: float = ALLY_FRONT_X if is_ally else ENEMY_FRONT_X
	var back_x: float = ALLY_BACK_X if is_ally else ENEMY_BACK_X
...
static func _assign_vertical_column(units: Array[Dictionary], column_x: float) -> void:
	...
	var start_y: float = MAGIC_CIRCLE_CENTER_Y - total_span * 0.5
...
static func _assign_single_group_positions(
	...
	for member_variant: Variant in group:
		...
		if unit_type == "player" and not back_used:
			member["slot_position"] = Vector2(back_x, group_y)
			back_used = true
			continue
		if not front_used:
			member["slot_position"] = Vector2(front_x, group_y)
			...
			overflow_index += 1
			var overflow_y: float = group_y + float(overflow_index) * 18.0
			member["slot_position"] = Vector2(back_x, overflow_y)
...
static func _apply_battle_area_layout(target: Dictionary, battle_area: Dictionary) -> void:
	...
	var ally_back_x: float = float(battle_area.get("ally_back_x", ALLY_BACK_X))
	var ally_front_x: float = float(battle_area.get("ally_front_x", ALLY_FRONT_X))
	var enemy_front_x: float = float(battle_area.get("enemy_front_x", ENEMY_FRONT_X))
	var enemy_back_x: float = float(battle_area.get("enemy_back_x", ENEMY_BACK_X))
