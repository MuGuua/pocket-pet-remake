...
@export var scene_display_name: String = "打怪区"
## level_scale_factor 是当前地图在移动端视口中的展示缩放倍数。
@export var level_scale_factor: float = 1.0
## 从学校 portal_id=5002 进入打怪区后的玩家场景坐标。
@export var inbound_from_xue_xiao_scene_position: Vector2 = Vector2(6.0, 10.0)
## login_and_map_teleport_spawn_position 是登录进入当前地图和世界地图快速传送共用的出生场景坐标。
@export var login_and_map_teleport_spawn_position: Vector2 = Vector2(6.0, 10.0)
...
## 根据传入打怪区的 portal_id 返回客户端场景坐标；未配置时返回无效坐标给上层兜底。
