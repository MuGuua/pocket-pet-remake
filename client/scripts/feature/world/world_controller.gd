...
func request_scene_transition(target_scene_id: int, portal_id: int = 0, facing_direction: Vector2 = Vector2.ZERO, map_teleport: bool = false) -> void:
	...
	var portal_spawn_scene_position: Vector2 = Vector2.INF
	if not map_teleport and portal_id > 0:
		portal_spawn_scene_position = _resolve_target_scene_portal_spawn_scene_position(target_scene_id, portal_id)
	...
	if portal_spawn_scene_position.is_finite():
		request_payload["target_pos"] = {
			"x": roundi(portal_spawn_scene_position.x),
			"y": roundi(portal_spawn_scene_position.y),
		}
...
func _apply_authoritative_snapshot() -> void:
	...
	# 普通门落点虽然由客户端目标场景脚本选定，但服务端验证后会持久化并回传同一坐标。
	# 此处统一消费 WORLD_RESYNC 的 self_pos，确保本人、旁观玩家与重连档案始终使用同一落点。
...
## 解析普通门进入目标场景后的出生场景坐标。
## target_scene_id 是即将加载的目标场景 ID；portal_id 是来源场景触发的门编号；返回无效值表示目标场景尚未配置该入口。
func _resolve_target_scene_portal_spawn_scene_position(target_scene_id: int, portal_id: int) -> Vector2:
	if target_scene_id <= 0 or portal_id <= 0:
		return Vector2.INF
	var scene_path: String = WorldSceneRegistry.get_scene_path(target_scene_id)
	if scene_path.is_empty():
		return Vector2.INF
	var level_scene: PackedScene = load(scene_path) as PackedScene
	if level_scene == null:
	...
	if level_instance == null:
		return Vector2.INF
	var spawn_position: Vector2 = _resolve_level_portal_spawn_scene_position(level_instance, portal_id)
	level_instance.free()
	return spawn_position


## 从一个目标地图根节点读取 portal_id 对应的出生场景坐标。
## level 是目标地图实例；portal_id 是来源门编号；返回无效值时由调用方回退到服务端坐标。
