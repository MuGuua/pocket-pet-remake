...
## 抽奖结束阶段每次移动一个点位的最长补间时长；配合更长过渡区间压缩纯慢速阶段。
@export_range(0.2, 1.2, 0.01) var draw_deceleration_seconds: float = 0.38
## 进入减速阶段前至少完整转动的圈数，避免结果刚好在当前位置时看不出轮盘转动。
@export_range(2, 10, 1) var draw_full_rounds: int = 3
## 减速阶段占总步数的比例；扩大过渡区间会同时压缩纯快速和纯慢速的体感时长。
@export_range(0.2, 0.65, 0.01) var draw_deceleration_ratio: float = 0.50
...
func _animate_next_draw_step() -> void:
	...
	# 使用五次 smootherstep 平滑衔接快、慢两端，让更多时间落在连续减速的过渡阶段。
	var eased_deceleration: float = deceleration_progress * deceleration_progress * deceleration_progress * (
		deceleration_progress * (deceleration_progress * 6.0 - 15.0) + 10.0
	)
