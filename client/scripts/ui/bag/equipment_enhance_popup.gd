...
## 放大后预览行左右边距，保持与强化面板 2 倍尺寸一致。
const PREVIEW_ROW_MARGIN_X: int = 12
## 放大后预览行最小高度，避免 24 号字体被裁切。
const PREVIEW_ROW_MIN_HEIGHT: int = 48
## 放大后预览行最大高度，用于限制动态行撑开布局。
const PREVIEW_ROW_MAX_HEIGHT: int = 52
## 放大后预览行属性名列宽。
const PREVIEW_ROW_STAT_WIDTH: int = 120
## 放大后预览行当前值列宽。
const PREVIEW_ROW_CURRENT_WIDTH: int = 160
## 放大后预览行箭头区域宽度。
const PREVIEW_ROW_ARROW_WIDTH: int = 48
## 放大后预览行下级值列宽。
const PREVIEW_ROW_NEXT_WIDTH: int = 296
## 放大后预览行字体大小。
const PREVIEW_ROW_FONT_SIZE: int = 24
## 放大后预览行内部横向间距。
const PREVIEW_ROW_SEPARATION: int = 16
## 强化进度条动画时长（秒）。
...
func _build_preview_row(row: Dictionary) -> MarginContainer:
	...
	row_margin.add_theme_constant_override("margin_left", PREVIEW_ROW_MARGIN_X)
	row_margin.add_theme_constant_override("margin_right", PREVIEW_ROW_MARGIN_X)
	var row_panel: PanelContainer = PanelContainer.new()
	row_panel.custom_minimum_size = Vector2(0, PREVIEW_ROW_MIN_HEIGHT)
	row_panel.custom_maximum_size = Vector2(-1, PREVIEW_ROW_MAX_HEIGHT)
	...
	row_hbox.add_theme_constant_override("separation", PREVIEW_ROW_SEPARATION)
	var stat_label: Label = Label.new()
	stat_label.custom_minimum_size = Vector2(PREVIEW_ROW_STAT_WIDTH, 0)
	stat_label.text = str(row.get("label", ""))
	stat_label.add_theme_font_size_override("font_size", PREVIEW_ROW_FONT_SIZE)
	stat_label.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	row_hbox.add_child(stat_label)
	var current_label: Label = Label.new()
	current_label.custom_minimum_size = Vector2(PREVIEW_ROW_CURRENT_WIDTH, 0)
	current_label.text = str(row.get("current", "+0"))
	current_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	current_label.add_theme_font_size_override("font_size", PREVIEW_ROW_FONT_SIZE)
	...
	arrow_icon.custom_minimum_size = Vector2(PREVIEW_ROW_ARROW_WIDTH, 0)
	...
	next_label.custom_minimum_size = Vector2(PREVIEW_ROW_NEXT_WIDTH, 0)
	next_label.text = next_text
	next_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	next_label.add_theme_font_size_override("font_size", PREVIEW_ROW_FONT_SIZE)
...
func _handle_modal_dismiss_input(event: InputEvent) -> void:
	...
	if _try_close_material_picker_for_event(event):
		return
	if _try_handle_direct_button_event(event):
		return
	if not _is_dismiss_event(event):
		return
...
## 兜底处理关键按钮点击；部分父级容器为了穿透遮罩会忽略鼠标，移动端上可能导致 Button pressed 信号丢失。
func _try_handle_direct_button_event(event: InputEvent) -> bool:
	if not _is_dismiss_event(event):
		return false
	var global_pos: Vector2 = _event_global_position(event)
	if global_pos.x < 0.0:
		return false
	if _is_button_event_on(_top_close_button, global_pos):
		get_viewport().set_input_as_handled()
		_on_top_close_button_pressed()
		return true
	if _is_button_event_on(_enhance_button, global_pos):
		get_viewport().set_input_as_handled()
		_on_enhance_button_pressed()
		return true
	return false


## 判断点击坐标是否落在按钮有效区域内；隐藏或禁用按钮不响应兜底点击。
func _is_button_event_on(button: BaseButton, global_pos: Vector2) -> bool:
	if button == null:
		return false
	if not button.visible or button.disabled:
		return false
	return button.get_global_rect().has_point(global_pos)
...
func _apply_panel_mouse_filters(node: Node, ignore: bool) -> void:
	if node is Control:
		...
		# 弹窗默认让非交互区域穿透到遮罩，关闭按钮必须保留 STOP 才能收到 pressed 信号。
		if control == _top_close_button or control == _material_select_button or control == _enhance_button:
