...
func _apply_visual_state() -> void:
	if _bg_rect == null:
		return
	var contains_equipment: bool = not _equipment.is_empty()
	_bg_rect.texture = texture_equipped if contains_equipment else texture_empty
	if _placeholder_rect != null:
		_placeholder_rect.visible = not contains_equipment
