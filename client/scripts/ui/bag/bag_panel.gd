...
## 详情面板与锚点按钮之间的垂直间距（像素）。
const DETAIL_ANCHOR_GAP_Y: float = 12.0
...
func _ensure_detail_overlay() -> void:
	...
	if not _detail_panel.close_requested.is_connected(_on_detail_panel_close_requested):
		_detail_panel.close_requested.connect(_on_detail_panel_close_requested)


## 详情遮罩收到按下输入时只吞掉事件，不再自动关闭详情层。
...
## 响应详情面板右上角关闭按钮，沿用现有详情层收起逻辑。
func _on_detail_panel_close_requested() -> void:
	_hide_detail_popup()
	_selected_equip_slot_key = ""
	_apply_selected_slot_visuals()
