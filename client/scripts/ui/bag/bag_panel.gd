...
## 容量展示标签。
@onready var _capacity_label: Label = $RootPanel/Margin/RootVBox/Header/CapacityLabel
## 关闭按钮。
@onready var _close_button: Button = $RootPanel/Margin/RootVBox/Header/CloseButton
## 格子网格。
@onready var _slot_grid: GridContainer = $RootPanel/Margin/RootVBox/SlotScroll/SlotGrid
## 物品详情面板。
@onready var _detail_panel: BagItemDetail = $RootPanel/Margin/RootVBox/DetailPanel
## 提示信息标签。
@onready var _message_label: Label = $RootPanel/Margin/RootVBox/MessageLabel
## 通用服务端请求 loading 遮罩。
@onready var _request_loading: RequestLoadingOverlay = $BagRequestLoadingOverlay
## 当前等待的背包请求序列号。
...
func _ready() -> void:
	...
	if _detail_panel != null and not _detail_panel.action_requested.is_connected(_on_detail_action_requested):
