...
## 本地固定过场对白支持回车、主键盘 5 和小键盘 5 推进，且不会触发服务端剧情请求。
func _input(event: InputEvent) -> void:
    if not _local_dialogue_active or not visible or _input_locked:
        return
    if event is not InputEventKey:
        return
    var key_event: InputEventKey = event as InputEventKey
    if not key_event.pressed or key_event.echo or not _is_local_advance_key(key_event):
        return
...
func _apply_content_text(full_text: String, content_format: String) -> void:
    ...
    if resolved_format == "bbcode":
        ...
        _content_label.append_text(full_text)
    else:
        _content_label.bbcode_enabled = false
        _content_label.add_text(full_text)
