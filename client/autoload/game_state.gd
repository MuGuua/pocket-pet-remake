...
## 每个远端实体最近接受的场景代次与移动序号，用于拒绝重复、乱序和跨场景延迟包。
var _remote_entity_move_versions: Dictionary = {}
# 当前拥有的宠物实例列表。
var pets: Array = []
# 当前编队中的宠物摘要列表。
var lineup: Array = []
...
func set_world_snapshot(payload: Dictionary) -> void:
    ...
    nearby_entities = {}
    # 全量世界快照会重建全部远端实体，因此必须同时丢弃上一张快照的移动版本基线。
    _remote_entity_move_versions = {}
...
## 向附近实体表中写入一个新实体；已存在实体只刷新服务端摘要，不重置已接受的移动序号。
func add_entity(entity: Dictionary) -> void:
    if not entity.has("entity_id"):
        return

    var entity_id: int = int(entity["entity_id"])
    var entity_already_exists: bool = nearby_entities.has(entity_id)
    if not entity_already_exists:
        # 真正的新实体不应继承同一 ID 上一次离场前的版本基线；编队刷新复用进入推送时则必须保留基线。
        _remote_entity_move_versions.erase(entity_id)
    var next_entity: Dictionary = entity.duplicate(true)
    var previous_entity_variant: Variant = nearby_entities.get(entity_id, {})
    if previous_entity_variant is Dictionary:
        var previous_entity: Dictionary = previous_entity_variant as Dictionary
        # 编队刷新复用 ENTITY_ENTER_PUSH；仅保留新摘要没有携带的短时移动表现，其他字段仍以新摘要完整替换。
        for transient_key: String in ["precise_pos", "facing", "moving"]:
            if not next_entity.has(transient_key) and previous_entity.has(transient_key):
                next_entity[transient_key] = previous_entity[transient_key]
    nearby_entities[entity_id] = next_entity
    world_snapshot_changed.emit()

## 从附近实体表中删除指定实体，并清理该实体的移动版本基线。
func remove_entity(entity_id: int) -> void:
    nearby_entities.erase(entity_id)
    _remote_entity_move_versions.erase(entity_id)
    world_snapshot_changed.emit()

## 把仍然新鲜的实体移动推送合并到本地快照；返回 false 表示该包无效、重复或已经倒退。
func apply_entity_move(payload: Dictionary) -> bool:
    var entity_id: int = int(payload.get("entity_id", 0))
    if entity_id <= 0 or not nearby_entities.has(entity_id):
        # 实体离场后到达的延迟包不能重新创建已经删除的远端角色。
        return false

    var entity_variant: Variant = nearby_entities.get(entity_id, {})
    if not (entity_variant is Dictionary):
        return false
    var current_scene_id: int = int(scene_snapshot.get("scene_id", 0))
    var payload_scene_id: int = int(payload.get("scene_id", current_scene_id))
    var snapshot_scene_version: int = int(scene_snapshot.get("scene_version", 0))
    var payload_scene_version: int = int(payload.get("scene_version", snapshot_scene_version))
    var move_seq: int = int(payload.get("move_seq", 0))
    if not _try_advance_remote_entity_move_version(
        entity_id,
        current_scene_id,
        payload_scene_id,
        snapshot_scene_version,
        payload_scene_version,
        move_seq
    ):
        return false

    var entity: Dictionary = (entity_variant as Dictionary).duplicate(true)
    ...
    if position_variant is Dictionary:
        entity["pos"] = (position_variant as Dictionary).duplicate(true)
    ...
    # 高精度坐标、明确朝向和移动状态只负责远端实时表现，不替代整数持久化坐标。
    var precise_position_variant: Variant = payload.get("precise_pos", {})
    if precise_position_variant is Dictionary:
        entity["precise_pos"] = (precise_position_variant as Dictionary).duplicate(true)
    var facing_variant: Variant = payload.get("facing", {})
    if facing_variant is Dictionary:
        entity["facing"] = (facing_variant as Dictionary).duplicate(true)
    entity["moving"] = bool(payload.get("moving", false))
...
## 校验单个远端实体的场景代次与移动序号，并在接受时原子替换本地版本基线。
## scene_version 为零时按当前世界快照代次兼容旧服务；move_seq 必须为正数，防止无序旧协议绕过过滤。
func _try_advance_remote_entity_move_version(
    entity_id: int,
    current_scene_id: int,
    payload_scene_id: int,
    snapshot_scene_version: int,
    payload_scene_version: int,
    move_seq: int
) -> bool:
    if current_scene_id <= 0 or payload_scene_id != current_scene_id or move_seq <= 0:
        return false

    var resolved_scene_version: int = payload_scene_version
    if resolved_scene_version <= 0:
        resolved_scene_version = snapshot_scene_version
    if snapshot_scene_version > 0 and resolved_scene_version < snapshot_scene_version:
        # 全量重同步后的旧场景代次即使尚无实体基线，也不能覆盖新快照。
        return false

    var previous_version_variant: Variant = _remote_entity_move_versions.get(entity_id, {})
    if previous_version_variant is Dictionary:
        var previous_version: Dictionary = previous_version_variant as Dictionary
        var previous_scene_id: int = int(previous_version.get("scene_id", 0))
        var previous_scene_version: int = int(previous_version.get("scene_version", 0))
        var previous_move_seq: int = int(previous_version.get("move_seq", 0))
        if previous_scene_id == payload_scene_id:
            if resolved_scene_version < previous_scene_version:
                return false
            if resolved_scene_version == previous_scene_version and move_seq <= previous_move_seq:
                return false

    _remote_entity_move_versions[entity_id] = {
        "scene_id": payload_scene_id,
