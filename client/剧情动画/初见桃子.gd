...
func _run_sequence() -> void:
    ...
    # 桃子：你怎么这么慢慢吞吞的啊？是不是想人七色羽给你点厉害尝尝～～
    await _show_actor_dialogue("桃子", "你怎么这么慢慢吞吞的啊？是不是想人七色羽给你点厉害尝尝～～", _get_animated_sprite_portrait(_taozi_sprite))

    # 玩家：别逗了！我会被七色羽的火焰烤熟的！
    await _show_actor_dialogue("玩家", "别逗了！我会被七色羽的火焰烤熟的！", _get_scene_player_portrait(), true)

    # 玩家：明天你就要去比武大会了，你们准备的怎么样了？有可能拿到名次不？
    await _show_actor_dialogue("玩家", "明天你就要去比武大会了，你们准备的怎么样了？有可能拿到名次不？", _get_scene_player_portrait(), true)

    # 桃子：挨挨.......你也太小看我了吧？让你看看我新训练的七色羽的大招吧，凤凰神炎！
    await _show_actor_dialogue("桃子", "挨挨.......你也太小看我了吧？让你看看我新训练的七色羽的大招吧，凤凰神炎！", _get_animated_sprite_portrait(_taozi_sprite))
    ...
    # 桃子：真棒！我就知道七色羽你绝对是这片大陆上最强的宠物精灵！咱们一定会再比武大会上获得冠军的
    await _show_actor_dialogue(
        "桃子",
        "真棒！我就知道七色羽你绝对是这片大陆上最强的宠物精灵！咱们一定会再比武大会上获得冠军的",
        _get_animated_sprite_portrait(_taozi_sprite)
    )

    # 玩家向左移动 10px，移动结束后面朝上。
    ...
    # 玩家：原来你们已经达到这种级别的实力了！太厉害了，比武大会的时候我会去给你加油的！
    await _show_actor_dialogue(
        "玩家",
        "原来你们已经达到这种级别的实力了！太厉害了，比武大会的时候我会去给你加油的！",
    ...
    # 桃子：什么加油？你在说什么啊？你也得和我一起去参加比武大会。你也得参加，和我一起参赛！明白吗？
    await _show_actor_dialogue(
        "桃子",
        "什么加油？你在说什么啊？你也得和我一起去参加比武大会。你也得参加，和我一起参赛！明白吗？",
        _get_animated_sprite_portrait(_taozi_sprite)
    )
...
## 从 AnimatedSprite2D 当前动画帧提取头像纹理，不修改角色正在播放的动画状态。
...
## 获取剧情场景中玩家当前实际显示的形象；优先动态皮肤，再回退旧版精灵图当前帧。
...
func _move_taozi_and_qiseyu_right() -> void:
    var relative_offset: Vector2 = Vector2(75.0, 0.0)
